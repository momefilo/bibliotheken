...
static void (*callbackMHz)(float_t mhz, uint8_t rssi, uint8_t flags);
...
void si4703_germanDefault_setup( \
		...
		void (*cbMHz)(float_t mhz, uint8_t rssi, uint8_t flags)){
	...
	si4703_space(0x02);// 50kHz Kanalbreite
...
void si4703_seeking(bool up){
	...
	while(gpio_get(GP2));
	busy_wait_ms(5);
	float_t mhz = si4703_getMHz();// reads the Register
...
void si4703_tuning(float_t mhz){
	...
	float_t f_chan = (mhz - ofset) / space;
	...
	while(gpio_get(GP2));
	busy_wait_ms(60);
	float_t real_mhz = si4703_getMHz();// reads the Register
	uint8_t rssi = REGISTER[17];
	uint8_t flags = getFlag();
	si4703_tune(false);
	si4703_write_register(4);
